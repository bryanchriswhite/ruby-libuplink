...
# i.e. `require 'ruby-libuplink/...'`
require_relative './lib/arg_forwarding_task'
...
begin
  ...
  LibUplink::ArgForwardingTask.new :spec,
                                   alias_names: [:test],
                                   args_deps_hash: {
...
Rake::ExtensionTask.new 'ruby-libuplink'

# TODO: everything that follows... but better
LibUplink::ArgForwardingTask.new(:build, args_deps_hash: {
    %i[no-test] => [TEMP_PATH.to_sym]
}) do |t, args, deps|
  Rake::Task[:spec].invoke if args.to_hash[:'no-test'].nil?

  sh 'gem build ruby-libuplink.gemspec'
  sh "mv ruby-libuplink-*.gem #{TEMP_PATH}#{File::SEPARATOR}"
end

LibUplink::ArgForwardingTask.new(:install, args_deps_hash: {
    %i[no-test] => []
}) do |t, args|
  Rake::Task[:build].invoke(*args.to_a)

  sh 'gem install --local ./tmp/ruby-libuplink-*.gem \
                        --no-ri \
                        --no-rdoc'
end
