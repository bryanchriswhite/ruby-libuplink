...
[![Gem Version](https://badge.fury.io/rb/ruby-libuplink.svg)](https://badge.fury.io/rb/ruby-libuplink)
[![Chat on rocket.chat](https://img.shields.io/badge/chat-rocket.chat-red.svg)](https://community.storj.io)

Ruby library for encrypted file transfer on the Storj network via bindings to [libuplink](https://github.com/Storj/libuplink).

## Using ruby-libuplink

### First, install [`libuplink`](https://github.com/storj/libuplink)
...
+ using rubygems (`gem`):
    ```bash
    gem install ruby-libuplink
    ```
+ from source:
    ```bash
    git clone https://github.com/storj/ruby-libuplink && \
    cd ruby-libuplink && \
    rake install
    ```
...
+ [`LibUplink`](./spec/libuplink_spec.rb) _[(source)](lib/ruby-libuplink.rb)_
    - `LibUplink::Ext::Storj::Mixins`  _[(source)](lib/mixins/storj.rb)_
        - `.util_timestamp`
        - `.util_datetime`
        - `.mnemonic_check`
        - `.mnemonic_generate`
+ [`LibUplink::Env`](spec/uplink_spec.rb) _[(source)](lib/uplink.rb)_
    - `.new`
    - `#get_info`
    - `#get_buckets`
    - `#create_bucket`
...
+ with `gem`:
    ...
    ```bash
    gem build ./ruby-libuplink.gemspec
    ```
    ```bash
    gem install --local ./ruby-libuplink-<version>.gem  --no-ri --no-rdoc  
    ```

### Testing:
#### First create spec/helpers/options.yml !
For the moment, the test suite doesn't start it's own mock backend but it does parse whatever's in the `spec/helpsers/options.yml` file to initialize `LibUplink::Ext::Storj::Env` to connect via http/https.
